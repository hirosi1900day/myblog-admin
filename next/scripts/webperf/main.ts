...
import { LaunchedChrome } from 'chrome-launcher/dist/chrome-launcher'

// const
// FIXME: constants.ts に移動させたい
const CHROME_OPTIONS: chromeLauncher.Options = {
  logLevel: 'info',
  chromeFlags: [
    '--headless',
    '--no-sandbox',
...
const fetchChromeVersion = async (chrome: LaunchedChrome) => {
  try {
...
const main = async () => {
  ...
  const chrome = await chromeLauncher.launch(CHROME_OPTIONS as Options)
  ...
  for (const url of TARGET_URLS) {
    ...
    const runnerResult = await lighthouse(
      url,
      { ...CHROME_OPTIONS, port: Number(port) },
      LIGHTHOUSE_OPTIONS
    )
    ...
    writeFileSync('./lhreport.html', reportHtml as string)

    const audits = JSON.parse(reportJson as string).audits
